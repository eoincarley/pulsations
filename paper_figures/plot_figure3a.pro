...
pro plot_figure3a, postscript=postscript
    ...
    ; Plots a three color NRH image on top of HMI.
    ; Overplots magnetic field lines from a PFSS extrapolation.

    loadct, 0
    window, 0, xs=1000, ys=1000, retain=2
    root_dir = '~/Data/2014_apr_18/pulsations/'
	restore,  root_dir+'hmi/hmi_map_20140418.sav'
	hmi_map = map

  	freqs = [2,4,5] ; 228, 298 and 327 MHz
  	x_size = 500 
  	y_size = 500
    border = 200
	...
	folder = root_dir+'radio_nrh_hi_res/' 
	cd, folder
   ...
   ; hmi_data[where(hmi_data ge 2.5e2)] = 5e2	; If you want to enhance the white.
    ...
    if keyword_set(postscript) then setup_ps, root_dir+'hmi_nrh_zoom1_20140818.eps', 1200, 1200
        loadct, 0, /silent
        gamma_ct, 0.6
	...
	restore, root_dir+'pfss_line_QAR_20140418.sav'

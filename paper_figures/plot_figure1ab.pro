...
;-
    ...
    !p.charsize = 1.5
    if ~keyword_set(img_name) then img_name = 'imag_000.eps'
    if ~keyword_set(img_tim) then img_tim = '2014-04-18T12:54:25'
    ...
    if keyword_set(zoom) then begin
        ...
        ;FOV = [35.0, 35.0]
        ;CENTER = [-1100.0, 400.0]
        FOV = [25, 25.0]  ;[25.0, 25.0] ;[20, 20]  ;[27.15, 27.15]    ;  [40., 40.]   ;[16.0, 16.0]    ;   [10, 10]  ;[16.0, 16.0]  ;[10, 10]    ;[27.15, 27.15];
        CENTER = [450, -450] ;[150, -200]  ;[450, -450] ;[-1100, 400]    ;[500, -350]    ;  [-900., 0.] ;[550, -230]  ;[600.0, -220] ;[500.0, -230]  ;[600.0, -220] ; 
    ...
    ;        *********************************
    ;-------------------------------------------------;
    first_img_index = closest(min_tim, anytim(img_tim, /utim))
    last_img_index = closest(min_tim, anytim('2014-04-18T018:35:00', /utim)) ; not used here.
    ...
    ENDIF ELSE BEGIN
        ...
        aia_process_fig1, fls_211[i], fls_211[i-5], i_a, i_a_pre, iscaled_a, im_type, imsize = array_size
        aia_process_fig1, fls_193[i], fls_193[i-5], i_b, i_b_pre, iscaled_b, im_type, imsize = array_size
        aia_process_fig1, fls_171[i], fls_171[i-5], i_c, i_c_pre, iscaled_c, im_type, imsize = array_size
    ENDELSE
    ...
    if keyword_set(postscript) then $
        ...
        setup_ps, '~/Desktop/'+eps_name, x_size+border, y_size+border
        ...
        nrh_process_fig1,  i_c.date_obs;, /freq_tags;, /back_sub            ; For the 2014-April-Event

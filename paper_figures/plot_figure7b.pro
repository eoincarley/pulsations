...
pro plot_figure7b, angle, postscript=postscript, choose_points=choose_points, $
				rratio=rratio, bratio=bratio, bdiff=bdiff, rdiff=rdiff, totalb=totalb, hot=hot

	; Code to plot the distance time maps from AIA, Repetition of aia_dt_plot_three_color.pro
	; Should be generalised to take in any set of dtmaps regardless of the date.

	; For Figure 7b of paper run aia_dtplot_gcircle_three_color, /rratio, /hot
	...
	folder = '~/Data/2014_apr_18/sdo/'	;'~/Data/2014_Apr_18/sdo/'	;'~/Data/2015_nov_04/sdo/event1/'
	
	if keyword_set(postscript) then begin
		set_plot, 'ps'
		device, filename='~/Desktop/aia_dt_maps_gcircles_20140418.eps', $
				/encapsulate, $
				/color, $ 
				/inches, $
				/helvetica, $
	...
	;
	;angle = '020'
	fileend = '_dt_map_gcircle_south.sav' ; great circiles that pass directly over the xpoint. 
	if ~keyword_set(hot) then waves = ['211', '193', '171']	else waves=['335', '131', '094']
	cd, folder+'/dist_time/great_circles/'
	
	restore,'aia_'+waves[0]+fileend
	t_a = dt_map_struct.time
	lindMm = dt_map_struct.distance	
	distt_a = dt_map_struct.dtmap
	if keyword_set(totalb) then for i=0, n_elements(lindMm)-1 do distt_a[*,i] = ( distt_a[*,i] - mean(distt_a[*,i]) ) /stdev(distt_a[*,i] )   

	restore,'aia_'+waves[1]+fileend
	t_b = dt_map_struct.time
	distt_b = dt_map_struct.dtmap
	if keyword_set(totalb) then for i=0, n_elements(lindMm)-1 do distt_b[*,i] = ( distt_b[*,i] - mean(distt_b[*,i]) ) /stdev(distt_b[*,i] )   


	restore,'aia_'+waves[2]+fileend
	t_c = dt_map_struct.time
	distt_c = dt_map_struct.dtmap
	if keyword_set(totalb) then for i=0, n_elements(lindMm)-1 do distt_c[*,i] = ( distt_c[*,i] - mean(distt_c[*,i]) ) /stdev(distt_c[*,i] )   
	...
	if keyword_set(hot) then begin	
		...
		max_scl = 1.1
	endelse
	if keyword_set(totalb) then begin
	...
	if keyword_set(rratio) then begin
	...
	if keyword_set(bratio) then begin
		for i=step_size, sizex-1 do begin
			plota[i, *] = alog( distt_a[i, *]/distt_a[0, *]  >min_scl < max_scl );^4	
			plotb[i, *] = alog( distt_b[i, *]/distt_b[0, *]  >min_scl < max_scl );^4
			plotc[i, *] = alog( distt_c[i, *]/distt_c[0, *]  >min_scl < max_scl );^4
		endfor
	endif

	if keyword_set(rdiff) then begin
		for i=step_size, sizex-1 do begin
			plota[i, *] = (distt_a[i, *]-distt_a[(i-step_size)>0, *])*1000. > (-1) < 1
			plotb[i, *] = (distt_b[i, *]-distt_b[(i-step_size)>0, *])*1000. > (-1) < 1
			plotc[i, *] = (distt_c[i, *]-distt_c[(i-step_size)>0, *])*1000. > (-1) < 1
		endfor
	endif
	if keyword_set(bdiff) then begin
		for i=step_size, sizex-1 do begin
			plota[i, *] = (distt_a[i, *]-distt_a[0, *])*1000. > (-5) < 1
			plotb[i, *] = (distt_b[i, *]-distt_b[0, *])*1000. > (-5) < 1
			plotc[i, *] = (distt_c[i, *]-distt_c[0, *])*1000. > (-5) < 1
		endfor
	endif

	plota = filter_image(plota, fwhm=2)
	plotb = filter_image(plotb, fwhm=2)
	plotc = filter_image(plotc, fwhm=2)
	

	tstart = anytim('2014-04-18T12:32:00', /utim) > min_tim[0] ;anytim('2014-04-18T12:00:00',/utim) > min_tim[0]
	tend = anytim('2014-04-18T13:15:00', /utim) < min_tim[n_elements(min_tim)-1] ;anytim('2014-04-18T13:10:00', /utim)	< min_tim[n_elements(min_tim)-1]
   ...
   ;stamp_date, 'AIA '+waves[0]+' '+cgsymbol('angstrom'), ' ', 'AIA '+waves[2]+' '+cgsymbol('angstrom')
	...
	if keyword_set(choose_points) then begin
		...
		print, 'Initial speed: '+string( p[1]*1e3 ) +' '+ string(perror[1])+' km/s'
		...
		set_line_color
		outplot, tsim+tim[0], ysim, linestyle=1, color=1, thick=6
	endif
	
	;outplot, tsim+tim[0], ysim, linestyle=1, color=1, thick=6
	;tsim = tsim+tim[0]
	;save, tsim, ysim, filename='~/idl/pulsations/dist_time/hot_wave_fit.sav'
	
	set_line_color
	restore,'~/idl/pulsations/dist_time/hot_wave_fit.sav', /verb
	outplot, tsim, ysim, linestyle=2, color=1, thick=2
	tim_marker1 = anytim('2014-04-18T12:54:00', /utim)
	tim_marker2 = anytim('2014-04-18T12:58:00', /utim)


	plots, [tim_marker1, tim_marker1], [0, hstop], linestyle=2, color=1, thick=5
	plots, [anytim('2014-04-18T12:32', /utim), anytim('2014-04-18T13:12', /utim)], [300, 300], linestyle=2, color=3, thick=5
	plots, [anytim('2014-04-18T12:32', /utim), anytim('2014-04-18T13:12', /utim)], [400, 400], linestyle=2, color=3, thick=5

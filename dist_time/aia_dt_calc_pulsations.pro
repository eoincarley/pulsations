pro aia_dt_calc_pulsations
	...
	winsz=1100
	AU = 1.49e11	; meters
	aia_waves = ['094A', '193A', '211A']	; ['094A', '131A', '335A']
	angles = [280.0]
	npoints = 300
	radius = 110	;arcsec
	x1 = 110.0
	...
	for k=0, n_elements(aia_waves)-1 do begin
		...
		window, 4, xs=winsz, ys=winsz, retain = 2, xpos=1900, ypos=1000
		...
		finish = closest(anytim(ind.date_obs, /utim), anytim('2014-04-18T13:10:00', /utim))
		...
		for j = 0, n_elements(angles)-1 do begin
			...
			WAVEL = string(he_dummy.WAVELNTH, format = '(I03)')
		  	...
		  	FOR i = start, finish DO BEGIN ;n_elements(f)-2 DO BEGIN
				...
				prof = interpolate(map.data, pixx, pixy)
			...
			dt_map_struct = {name:'dt_map_'+WAVEL, dtmap:distt, time:tarr, distance:lindMm, angle:angle, xyarcsec:[[xlin], [ylin]] }
		  	;save, dt_map_struct, $
		  ;		filename='~/Data/2014_apr_18/sdo/dist_time/aia_'+WAVEL+'_pulsations.sav'
		  	stop
	  	ENDFOR		
  	endfor
	STOP

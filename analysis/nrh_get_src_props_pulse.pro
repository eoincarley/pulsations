...
pro calculate_flux, source_section, freq, $
	...
	indices = where(source_section ge max(source_section)*0.5)
...
pro nrh_get_src_props_pulse, save_props=save_props
	...
	for file_num=1, 4 do begin
		winsize=500
		xpos=2050
		window, 0, xs=winsize, ys=winsize, xpos=xpos, ypos=1100, retain=2
		window, 1, xs=winsize, ys=winsize, xpos=xpos, ypos=1100-winsize*1.4, retain=2
		winsize=350
		window, 2, xs=winsize, ys=winsize, xpos=xpos-winsize*1.1, ypos=1100, retain=2
		window, 3, xs=winsize, ys=winsize, xpos=xpos-winsize*1.1, ypos=1100-winsize, retain=2
		window, 4, xs=winsize, ys=winsize, xpos=xpos-winsize*1.1, ypos=1100-winsize*2, retain=2
		window, 5, xs=winsize, ys=winsize, xpos=xpos-winsize*1.1, ypos=1100-winsize*3, retain=2
		...
		for img_num=0, n_elements(nrh_hdrs)-1 do begin		  
			...
			poor_src_sz = n_elements( where(junk_array gt 0.5*max(junk_array) ) )*nrh_hdrs[0].cdelt1^2.0	; Poor man's source size
			...
			if status le 0 or status eq 5 then begin	; If fit error then skip
				...
				totx = total(data_section>0.0, 2)
				mean_x = total( (findgen(n_elements(totx))+1.0)*totx)/total(totx)
				toty = total(data_section>0.0, 1)
				...
				set_line_color
				plots, source_max_x_arcs, source_max_y_arcs, psym=7, color=0, symsize=3, thick=1, /data
				plots, source_maxTb_x_arcs, source_maxTb_y_arcs, psym=1, color=0, symsize=3, thick=1, /data
				plots, src_posx_mean_arc, src_posy_mean_arc, psym=4, color=0, symsize=3, thick=1, /data
				loadct, 3, /silent
		...
		if keyword_set(save_props) then save, xy_arcs_struct, filename='~/Data/2014_apr_18/pulsations/nrh_'+freq_string+'_pulse_src_props_hires_si.sav', $
				description = 'Stokes I. XY coords in arc seconds. Made using nrh_get_src_props_pulse.pro'
	endfor			
